...
Feature: Creation of BOMStyle
  ...
  @Stylecolorways
  Scenario Outline: Verify Push Colorways to Style action for a Style
    #Given User launches centric application
    Then Go to homepage
    And verify user screen
    Then Verify push colorways to style and check another style
    Then Verify style editable and check code   "<Style>"    
    And Verify push colorway to style does not duplicate the colorway
    And Verify push colorway to style is not supported when the style type configure with has color is false
    
    Examples: 
      | Style  							 		|
      |11-0103 EGRET-edit,EE-001|