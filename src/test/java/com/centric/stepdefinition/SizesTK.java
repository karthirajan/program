...
try{
	...
	@Then("Create colorway along with Color Specification and Verify {string}")
	public void Create_colorway_along_with_Color_Specification_and_Verify(String color1) throws Throwable {
		Actions a = new Actions(driver);
		jsWaitForPageLoad();
		ca.eleToBeClickable();
		driver.navigate().refresh();
...
try{
	...
	// TODO: handle exception
		...
		ca.click(shp.getNowColorwayAction());
		ca.eleToBeClickable();
	    ca.click(lct.getFieldclrway());
		ca.eleToBeClickable();
		ca.insertText(lct.getFieldclrway1(), color1);
		ca.eleToBeClickable();
		ca.insertText(lct.getFieldclrway2(), color1);
		Commonactions.isElementPresent(cp.getSaveBtn());
		ca.click(cp.getSaveBtn());
		ca.eleToBeClickable();
		ca.click(lct.getPluginRefresh());
		ca.eleToBeClickable();
	}
	
	@Then("Create New Colorway by selecting a color specification already added for the Style and Verify error message is displayed {string}")
	public void Create_New_Colorway_by_selecting_a_color_specification_already_added_for_the_Style_and_Verify_error_message_is_displayed(String color1) throws Throwable {
		Commonactions.isElementPresent(shp.getNowColorwayAction());
		ca.click(shp.getNowColorwayAction());
		ca.eleToBeClickable();
		//ca.click(driver.findElement(By.xpath("//div[@data-csi-automation='field-Colorway-Form-ColorSpecification']//div[3]/input[1]")));
		
		ca.eleToBeClickable();
		ca.insertText(lct.getDivClrway1(), color1);
		ca.eleToBeClickable();
		//ca.click(driver.findElement(By.xpath("//div[@class='dijitReset dijitMenuItem' and text()='"+color1+"']")));
		ca.click(lct.getClrSpecfi());
		ca.eleToBeClickable();
		ca.insertText(lct.getFieldclrway1(), color1);
		ca.eleToBeClickable();
		Commonactions.isElementPresent(cp.getSaveBtn());
		ca.click(cp.getSaveBtn());
		Commonactions.isElementPresent(lct.getDivUniQue());
	...
	public void createCollectionByProvidingValidaAndMandatoryDetails(String a) throws Throwable {
		...
		try{
			...
			ca.click(lct.getRefreshView());
			ca.eleToBeClickable();
			ca.click(lct.getTdNodName());
